function [ x2shift, x2score ] = DynamicAlignXC( y1, y2, frameWidth, frameStep, alignRange, plotGraphs)
%DYNAMICALIGN Aligns a signal y2 to signal y1 by cross-correlating (XC) frames of y2, across all of y1
...
%
% If you want to return a sample-aligned version of y2, use DynamicAlignXCResamp instead
    ...
    for k = -alignRange:alignRange
        ...
        for m = 1:length(p)
            if (p1_k(m) > 0) & (p1_k(m) <= L1)
                ...
                %c = c + abs(y1(p1_k(m)) - y2(p(m))) .^2; %Squares of differences
            end
        end
...
if plotGraphs == 1
    %figure(204); 
    hold off;
    surf(-alignRange:alignRange, 0:npos-1, CM', 'LineStyle', 'none');
    view(0, 90);
    xlabel('lag');
    ...
    hold on;
    plot3(P-alignRange, 0.5 + (0:npos-1), ones(npos, 1) * 1, 'k+');
    hold on;
...
plot3(x2shift, (0:L2-1) / frameStep + 0.5, ones(L2, 1) * 1, 'r');
